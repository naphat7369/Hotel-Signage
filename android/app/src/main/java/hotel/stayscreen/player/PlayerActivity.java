...
public class PlayerActivity extends Activity {
 ...
 private Bitmap imageBitmap; private Runnable advance; private long lastVideoPosition=-1; private int stalled=0;
 ...
 private void startPlayer(){frame=new FrameLayout(this);frame.setBackgroundColor(Color.BLACK);setContentView(frame);picture=new ImageView(this);picture.setScaleType(ImageView.ScaleType.FIT_CENTER);frame.addView(picture,new FrameLayout.LayoutParams(-1,-1));texture=new TextureView(this);frame.addView(texture,new FrameLayout.LayoutParams(-1,-1));message=new TextView(this);message.setTextColor(Color.WHITE);message.setGravity(Gravity.CENTER);frame.addView(message,new FrameLayout.LayoutParams(-1,-1));message.setText("กำลังดาวน์โหลด Playlist…");
  manifest=readJson("manifest.json",null);tickProgram();network.scheduleWithFixedDelay(this::sync,0,30,TimeUnit.SECONDS);ui.postDelayed(this::clockTick,1000);
 }
 ...
 private void stopMedia(){generation++;if(advance!=null){ui.removeCallbacks(advance);advance=null;}if(video!=null){try{video.stop();}catch(Exception ignored){}try{video.reset();}catch(Exception ignored){}video.release();video=null;}if(surface!=null){try{surface.release();}catch(Exception ignored){}surface=null;}texture.setSurfaceTextureListener(null);picture.setImageDrawable(null);if(imageBitmap!=null){imageBitmap.recycle();imageBitmap=null;}item=null;lastVideoPosition=-1;stalled=0;}
 private void play(int wanted){stopMedia();if(program==null){message.setVisibility(View.VISIBLE);message.setText("รอ Schedule");return;}JSONArray list=program.optJSONArray("items");if(list==null||list.length()==0)return;index=Math.floorMod(wanted,list.length());item=list.optJSONObject(index);File file=new File(getFilesDir(),item.optString("media"));if(!file.exists()){message.setVisibility(View.VISIBLE);message.setText("Cache ไม่ครบ กำลังรอดาวน์โหลดซ่อม");advance=()->play(index+1);ui.postDelayed(advance,2500);return;}message.setVisibility(View.GONE);began=System.currentTimeMillis();final int serial=generation;
  ...
  if(!isVideo){picture.setVisibility(View.VISIBLE);picture.bringToFront();message.bringToFront();BitmapFactory.Options opts=new BitmapFactory.Options();opts.inJustDecodeBounds=true;BitmapFactory.decodeFile(file.getPath(),opts);opts.inSampleSize=1;while(opts.outWidth/opts.inSampleSize>1920||opts.outHeight/opts.inSampleSize>1080)opts.inSampleSize*=2;opts.inJustDecodeBounds=false;imageBitmap=BitmapFactory.decodeFile(file.getPath(),opts);picture.setImageBitmap(imageBitmap);advance=()->finishItem(serial);ui.postDelayed(advance,(long)(item.optDouble("duration",10)*1000));}
  else{
   ...
   Runnable ready=()->{
    ...
    try{
     ...
     if(st==null)throw new IllegalStateException("SurfaceTexture is null");
     ...
     video.setOnPreparedListener(p->{
      if(generation!=serial)return;
      fitVideo(p.getVideoWidth(),p.getVideoHeight());
      p.start();
     });
   ...
   texture.setSurfaceTextureListener(new TextureView.SurfaceTextureListener(){
    public void onSurfaceTextureAvailable(SurfaceTexture s,int w,int h){ui.post(ready);}
    public void onSurfaceTextureSizeChanged(SurfaceTexture s,int w,int h){}
    public boolean onSurfaceTextureDestroyed(SurfaceTexture s){return false;}
    public void onSurfaceTextureUpdated(SurfaceTexture s){}
   });
   if(texture.isAvailable()&&texture.getSurfaceTexture()!=null){ui.post(ready);}
  }
 }
 ...
 @Override protected void onDestroy(){super.onDestroy();ui.removeCallbacksAndMessages(null);network.shutdownNow();if(video!=null)video.release();}
}
