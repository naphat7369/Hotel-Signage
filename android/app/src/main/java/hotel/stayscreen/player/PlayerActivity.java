...
public class PlayerActivity extends Activity {
 ...
 private boolean active(JSONObject s){try{ZonedDateTime t=Instant.ofEpochMilli(System.currentTimeMillis()+offset).atZone(ZoneId.of(s.getString("timezone")));String currentStr=String.format("%04d-%02d-%02d %02d:%02d",t.getYear(),t.getMonthValue(),t.getDayOfMonth(),t.getHour(),t.getMinute());boolean isAllDay=s.getString("startTime").equals(s.getString("endTime"));String startStr=s.getString("startDate")+" "+s.getString("startTime");String endStr=s.getString("endDate")+" "+(isAllDay?"23:59":s.getString("endTime"));if(currentStr.compareTo(startStr)<0||currentStr.compareTo(endStr)>0)return false;int dow=t.getDayOfWeek().getValue()%7;boolean allowed=false;JSONArray ds=s.getJSONArray("days");for(int i=0;i<ds.length();i++)if(ds.getInt(i)==dow)allowed=true;return allowed;}catch(Exception e){return false;}}
